...
import 'package:bella_desktop/screens/product_category_details_screen.dart';
import 'package:bella_desktop/screens/product_category_edit_screen.dart';
...
class _ProductCategoryListScreenState extends State<ProductCategoryListScreen> {
  ...
  SearchResult<Category>? productCategories;
  int _currentPage = 0;
  int _pageSize = 5;
  ...
  Future<void> _deactivateCategory(Category category) async {
    ...
    try {
      // Create request with all category data, but set isActive to false
      final request = {
        'name': category.name,
      ...
      await categoryProvider.update(category.id, request);
  ...
  Widget build(BuildContext context) {
    return MasterScreen(
      title: "Product Categories Administration",
      child: Center(
        child: Column(
          children: [
            _buildSearch(),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const ProductCategoryEditScreen(),
                  settings: const RouteSettings(name: 'ProductCategoryEditScreen'),
                ),
              );
            },
            style: ElevatedButton.styleFrom(
  ...
  Widget _buildResultView() {
    final isEmpty = productCategories == null ||
        productCategories!.items == null ||
        productCategories!.items!.isEmpty;
    final int totalCount = productCategories?.totalCount ?? 0;
      ...
      child: Column(
        children: [
          BaseTable(
                          ...
                          DataCell(
                            Row(
                              ...
                              children: [
                                        ...
                                        Navigator.push(
                                          ...
                                          MaterialPageRoute(
                                            ...
                                            settings: const RouteSettings(
                                              name: 'ProductCategoryDetailsScreen',
                                            ),
                                          ),
                                        );
                                      },
                                          ...
                                          MaterialPageRoute(
                                            builder: (context) =>
                                                ProductCategoryEditScreen(
                                                    category: e),
                                            settings: const RouteSettings(
                                              name: 'ProductCategoryEditScreen',
                                            ),
                                          ),
                                        );
                                      },
                                ...
                                // Deactivate button - only show for active categories
                                if (e.isActive) ...[
                                  const SizedBox(width: 10),
                                  MouseRegion(
                                    cursor: SystemMouseCursors.click,
            ...
            emptySubtext:
                "Try adjusting your search or add a new product category.",
          ),
